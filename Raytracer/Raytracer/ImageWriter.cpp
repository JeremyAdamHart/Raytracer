...
void writePng(glm::vec3 *data, int width, int height, std::string filename) {
	...
	for (int i = 0; i < width*height; i++) {
		imageBytes[3 * i] = char(255.f*glm::clamp(data[i].x, 0.f, 1.f));
		imageBytes[3 * i + 1] = char(255.f*glm::clamp(data[i].y, 0.f, 1.f));
		imageBytes[3 * i + 2] = char(255.f*glm::clamp(data[i].z, 0.f, 1.f));
	}
